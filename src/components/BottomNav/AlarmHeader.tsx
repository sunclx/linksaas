        ...
        <div style={{ marginRight: "80px" }}>
            <Space size="middle">
                {(projectStore.curProject?.setting.hide_bulletin ?? false) == false && (
                    <Popover open={projectStore.curProject?.project_status.bulletin_count == 0 ? false : undefined} trigger="hover"
                        mouseLeaveDelay={1}
                        content={<BulletinList />} placement="topLeft" destroyTooltipOnHide autoAdjustOverflow>
                        <Badge count={projectStore.curProject?.project_status.bulletin_count} size="small" dot={appStore.focusMode}>
                            <BellTwoTone style={{ fontSize: "20px", cursor: "default" }} twoToneColor={projectStore.curProject?.project_status.bulletin_count == 0 ? "#ccc" : ["#aaa", "cyan"]} title="项目公告" />
                        </Badge>
                    </Popover>
                )}
