...
use proto_gen_rust::project_api::{
    ...
    GetLocalApiPermResponse, GetLocalApiTokenRequest, LocalApiPerm,
};
use tauri::AppHandle;

pub async fn check_token(app: &AppHandle, project_id: &String, token: &String) -> bool {
    if token == "" {
        return false;
    }
    ...
    let res = client
        .get_local_api_token(GetLocalApiTokenRequest {
